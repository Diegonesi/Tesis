...
def beam_search(num_items, capacidad, beneficios, pesos, matriz_relacion, tipo_funcion_haz, parametro_control, archivo_registro):
    ...
    for nivel in range(num_items):
        ...
        if nivel == 0:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=24) as executor:# Cambiar nodos a utilizar
                resultados = executor.map(procesar_nodo, [sol for _, sol in beam])

            for nuevos_candidatos in resultados:
                for beneficio, nueva_solucion, peso, elementos_usados in nuevos_candidatos:
...
ruta = ".\\Problemas\\Benchmark2.txt" #Cambiar problema
num_items, elementos, capacidad, beneficios, pesos, matriz_relaciones = Extraer_Datos.leer_datos_SUKP(ruta)

archivo_registro = "Resultados_Hilos_Benchmark2.txt" #Nombre del txt

...
def beam_search(num_items, capacidad, beneficios, pesos, matriz_relacion, tipo_funcion_haz, parametro_control, archivo_registro, min_pct, max_pct):
    ...
    for nivel in range(num_items):
        ...
        for beneficio_actual, solucion_actual in beam:
            for decision in [0, 1]:
                ...
                porcentaje_usado = peso / capacidad
                if peso > capacidad or beneficio == 0:
                    continue
                if (nivel + 1) >= 5 and not (min_pct <= porcentaje_usado <= max_pct):
                    continue
...
mejor_solucion, mejor_valor = beam_search(num_items, capacidad, beneficios, pesos, matriz_relaciones, 'log', 5, archivo_registro, 0.5, 1 ) # Solo considerar soluciones con entre 50% y 100% de capacidad usada
